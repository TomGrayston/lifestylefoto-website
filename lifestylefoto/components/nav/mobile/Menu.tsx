import { ReactNode } from "react";

import { BsXLg as Icon }  from "react-icons/bs";

interface Props {
    children?: ReactNode,
    open: boolean,
    setOpen : any,
    bgColour: string,
    textColour: string
}

const Menu = ({children, open, setOpen, bgColour, textColour}: Props) => {
    return(
        <div className={`absolute top-0 right-0 bottom-0 h-screen transition-all w-full ${!open ? '-left-full opacity-0' : 'left-0 opacity-100'} ${bgColour} ${textColour}`}>
            <div className="flex items-center justify-end px-2 py-2 m-auto">
                <button className="px-1 py-1 text-3xl bg-inherit hover:text-stone-200 text-inherit" onClick={() => setOpen(false)}>
                    <Icon/>
                </button>
            </div>
            {children}
        </div>
    );
}

export default Menu;